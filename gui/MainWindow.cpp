...
MainWindow::MainWindow() : window(sf::VideoMode(800, 600), "CSC Organization Audit") {
    ...
    // Labels
    samisLabel.setFont(font);
    samisLabel.setFillColor(sf::Color(60, 60, 70));
    samisLabel.setString("SAMIS File");
    samisLabel.setCharacterSize(18);
    samisLabel.setPosition(115, 100);
    ...
    // input boxes
    samisInputBox.setSize({450, 40});
    samisInputBox.setPosition(115, 125);
    samisInputBox.setFillColor(sf::Color::White);
    samisInputBox.setOutlineThickness(2.f);
    samisInputBox.setOutlineColor(sf::Color(180, 185, 195));
    ...
    // Input Text
    samisInputText.setFont(font);
    samisInputText.setFillColor(sf::Color::Black);
    samisInputText.setCharacterSize(16);
    samisInputText.setPosition(125, 135);
    ...
    // Browse buttons
    samisBrowseButton.setSize({120, 40});
    samisBrowseButton.setPosition(580, 125);
    samisBrowseButton.setFillColor(sf::Color(52, 120, 246));
    samisBrowseButton.setOutlineThickness(2.f);
    samisBrowseButton.setOutlineColor(sf::Color(25, 80, 180));
    ...
    samisBrowseText.setFont(font);
    samisBrowseText.setFillColor(sf::Color::White);
    samisBrowseText.setString("Browse");
    samisBrowseText.setCharacterSize(16);
    samisBrowseText.setPosition(605, 135);
...
void MainWindow::handleEvents() {
    ...
    while (window.pollEvent(event)) {
        ...
        if (event.type == sf::Event::MouseButtonPressed) {
            ...
            if (samisInputBox.getGlobalBounds().contains(mouse)) {
                activeField = 1;
                samisCursorPosition = samisPath.length();
            ...
            if (samisBrowseButton.getGlobalBounds().contains(mouse)) {
            ...
            if (startButton.getGlobalBounds().contains(mouse)) {
                if (samisPath.empty()) {
                    MessageBoxA(
                        nullptr,
                        "Please select the SAMIS file for the desired year.",
                        "Missing Information",
        ...
        if (event.type == sf::Event::KeyPressed) {
            ...
            if (event.key.control && event.key.code == sf::Keyboard::V) {
                ...
                if (activeField == 1) {
                    samisPath.insert(samisCursorPosition, paste);
                    samisCursorPosition += paste.length();
                }
            ...
            if (event.key.code == sf::Keyboard::Left) {
                if (activeField == 1 && samisCursorPosition > 0)
                    samisCursorPosition--;
            ...
            if (event.key.code == sf::Keyboard::Right) {
                if (activeField == 1 && samisCursorPosition < samisPath.length())
                    samisCursorPosition++;
        ...
        if (event.type == sf::Event::TextEntered) {
            if (event.text.unicode < 128 && event.text.unicode >= 32) {
                ...
                if (activeField == 1) {
                    samisPath.insert(samisCursorPosition, 1, character);
                    samisCursorPosition++;
                }
...
void MainWindow::updateTextFields() {
    std::string samisDisplay = samisPath;
    std::string cscDisplay = cscOrganizationsPath;
    std::string yearDisplay = auditYear;

    if (activeField == 1) {
        samisDisplay.insert(samisCursorPosition, "|");
        samisInputBox.setOutlineColor(sf::Color(52, 120, 246));
    } else
        samisInputBox.setOutlineColor(sf::Color(180, 185, 195));
    ...
    samisInputText.setString(truncateText(samisDisplay));
