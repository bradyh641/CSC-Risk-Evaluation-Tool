...
void CombineResults::scanMemory(const std::vector<SAMISOrganization> &samis,
    ...
    // iterate through samis
    for (int i = 0; i < samis.size(); ++i) {
        if (map.contains(samis[i].organizationName)) {
...
CombineResults::matchOrganizations(
    ...
    scanMemory(samis, audits, matches, samisMatched, auditMatched);

    for (size_t i = 0; i < samis.size(); i++) {
        if (samisMatched[i])
            continue;
